# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 13:53:07 2026
...
# Path to input shapefile (fire perimeter)
SHAPEFILE_PATH = r"C:\path to your perimeter\FirePerimeter.shp"

# Output root folder: same as shapefile folder
OUTPUT_DIR = r"D:\DEMs"
...
def main():
    """Query TNM DEM products for a fire perimeter shapefile and download files."""
    ...
    for idx, sr in enumerate(reader.shapeRecords(), 1):
        ...
        if not urls:
            print(f"No download URLs parsed for polygon {idx}")
            continue

        print(f"Preparing to download {len(urls)} files for polygon {idx}")
        if DRY_RUN:
            for u in urls:
                print(u)
            continue
